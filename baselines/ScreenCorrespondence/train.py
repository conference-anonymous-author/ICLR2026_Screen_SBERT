    ...
    for epoch in range(max_epoch):
        ...
        for batch in dataloader_train:         
            ...
            for bi in range(len(screen_indices)):
                ...
                types.append(torch.tensor(np.load(f"../../dataset/{app}/{si}/gui_functional_types.npy")))
                visions.append(torch.tensor(np.load(f"../../dataset/{app}/{si}/gui_vision_feature_maps.npy")))
                texts.append(torch.tensor(np.load(f"../../dataset/{app}/{si}/gui_text_embeds.npy")))
    
            padded_coords = pad_sequence(coords, batch_first=True).float().to(device)
            padded_types = pad_sequence(types, batch_first=True, padding_value=27).to(device)
