...
class PW2SS(nn.Module):
    ...
    def mask_predict(self, text_embed, text_coords, text_mask, graphic_types, graphic_coords, graphic_mask, layout):
        coords = torch.cat([text_coords, graphic_coords], dim=1)
        ...
        E_text = self.text_proj(text_embed)
        E_graphic = self.type_table(graphic_types)
        E_pw = torch.cat([E_text, E_graphic], dim=1)
        ...
        true_graphic = E_graphic[graphic_mask_indices]
    ...
    def encode(self, text_embed, text_coords, text_mask, graphic_types, graphic_coords, graphic_mask, layout):
        coords = torch.cat([text_coords, graphic_coords], dim=1)
        ...
        masked_enc = enc.masked_fill(~mask, float('-inf'))
    ...
    def sim_cosine(
        self, 
        text_embed1, text_coords1, text_mask1, graphic_types1, graphic_coords1, graphic_mask1, layout1,
        text_embed2, text_coords2, text_mask2, graphic_types2, graphic_coords2, graphic_mask2, layout2
    ):
        screen_emb1 = self.encode(text_embed1, text_coords1, text_mask1, graphic_types1, graphic_coords1, graphic_mask1, layout1)
        screen_emb2 = self.encode(text_embed2, text_coords2, text_mask2, graphic_types2, graphic_coords2, graphic_mask2, layout2)
