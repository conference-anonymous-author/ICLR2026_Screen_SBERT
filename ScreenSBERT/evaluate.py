        ...
        for app in evaluation_apps:
            ...
            for screen_indices in tqdm(page2screen_indices.values(), desc=f"Embedding {app}"):
                for screen_idx in screen_indices:                
                    ...
                    padding_mask = torch.ones(1, coords.size(1), dtype=torch.bool, device=device)
                    ...
                    embeddings[app][screen_idx] = screen_emb.detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load(f"./weights/ScreenSBERT/evaluate_X_Temu.pth", weights_only=True))
    model.eval()
